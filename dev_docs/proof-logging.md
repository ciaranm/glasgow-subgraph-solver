...
| `--locally-injective` needs `--no-supplementals` | the supplemental-graph derivations have no locally-injective variant yet (the degree and NDS filters do) |
...
(its closed neighbourhood if `v` has a self-loop). The degree and NDS pigeonholes use these in place
of the global injectivity constraints — wherever those derivations would sum "at most one pattern
vertex maps to this target", local injectivity instead sums "at most one of `v`'s neighbours maps to
this target", which is exactly what the argument needs (the neighbours of `v` are all it constrains).
The supplemental-graph derivations still need locally-injective variants, so for now local
injectivity additionally requires `--no-supplementals`.
